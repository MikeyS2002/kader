...
function specsLine(specs) {
  ...
  if (specs.ceilingM) parts.push(`plafond ${String(specs.ceilingM).replace(".", ",")} m`);
      ...
      switch (type) {
        ...
        case "fill": {
          ...
          if (/water|ocean|river|lake/.test(lid)) color = t.water;
            ...
            /park|grass|wood|forest|green|landcover|landuse|vegetation|cemetery|pitch|garden|sand/.test(lid)
          )
            color = t.green;
        ...
        case "line": {
          let color = t.road;
          if (/water|river|waterway|canal|stream/.test(lid)) color = t.water;
...
export function StudioMap() {
  ...
  const [types, setTypes] = useState({ Foto: true, Podcast: true, Muziek: true });
    ...
    const updateMarkers = () => {
      ...
      for (const [key, entry] of registry) {
        if (entry.dying && next.has(key)) {
          ...
          entry.inner.style.transform = "translate(0px, 0px) scale(1)";
        }
      }
      ...
      const removedPx = removed.map(([, entry]) => map.project(entry.lngLat));
      const nextPx = [...next.values()].map((f) => map.project(f.geometry.coordinates));
      ...
      for (const [key, f] of added) {
        ...
        el.addEventListener("click", (ev) => {
          ...
          if (props.cluster) {
            source
              .getClusterLeaves(props.cluster_id, props.point_count, 0)
              .then((leaves) => {
                const items = leaves
                  .map((l) => ({
                    ...l.properties,
                  ...
                  .sort(
                    (a, b) =>
                      a.city.localeCompare(b.city) || a.priceSort - b.priceSort
                  );
                setCard(null);
  ...
  const flyToPlace = (result) => {
    ...
    if (bb?.length === 4) {
      ...
      map.flyTo({ center: [Number(result.lon), Number(result.lat)], zoom: 12 });
    }
  };
  ...
  return (
    <div className="relative h-dvh w-full overflow-hidden bg-cyc">
      ...
      <div className="absolute left-[4.75rem] top-4 z-30">
        <PlaceSearch onPick={flyToPlace} onFocusInput={() => setCard(null)} />
      </div>
        ...
        {card && (
          ...
          >
            ...
            <div className="p-5">
              ...
              {card.specsLine ? (
                <Typography type="caption" as="p" className="mt-2 opacity-70">
                  {card.specsLine}
                </Typography>
              ) : null}
              {card.description ? (
              ...
              <div className="mt-3 flex items-baseline justify-between gap-3">
                {card.priceLabel ? (
                  <Typography type="spec" className="text-[13px] font-medium">
                    {card.priceLabel}
                  </Typography>
                ) : (
                  <span />
              ...
              <Typography type="caption" as="p" className="mt-2 opacity-40">
                via gearbooker.com
              </Typography>
            </div>
          </div>
        ...
        >
          <div className="flex items-start justify-between gap-3 px-6 pb-4 pt-6">
            <div>
              ...
              <Typography type="caption" as="div" className="mt-0.5 opacity-60">
            ...
            {sheet.items.map((item) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  {item.priceLabel ? (
                    <Typography type="caption" className="ml-auto opacity-80">
                      {item.priceLabel}
                    </Typography>
                  ) : null}
                </div>
                ...
                {item.specsLine ? (
                  <Typography type="caption" as="div" className="mt-1 opacity-60">
                    {item.specsLine}
                  </Typography>
                ) : null}
              </button>
    ...
    <div className="relative">
      <div className="flex h-11 items-center gap-2.5 rounded-full border border-line bg-white pl-4 pr-4 shadow-[0_10px_30px_-15px_rgba(23,23,22,0.35)]">
        <svg
          ...
          aria-hidden
        >
          <circle cx="7" cy="7" r="5" stroke="#171716" strokeWidth="1.6" />
          <path d="M11 11 L14.5 14.5" stroke="#171716" strokeWidth="1.6" strokeLinecap="round" />
        </svg>
        <input
          type="text"
          value={query}
